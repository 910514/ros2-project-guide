...
class TurtleBot3YOLO(Node):
    def __init__(self):
        ...
        # Initialize movement state and timer
    ...
    def scan_callback(self, scan):
        ...
        if cnt > 0:
            self.latest_distance = total_distance / cnt
        else:
            self.latest_distance = None
    ...
    def process_image(self):
        if self.latest_image is None or self.is_rotating or self.is_moving:
            return
    ...
    def move_towards_target(self):
        if self.latest_distance is not None and self.latest_distance > 0.2:  # Move towards the target if it's more than 0.2 meters away
            twist = Twist()
            twist.linear.x = -0.05  # Move forward
            self.cmd_vel_publisher.publish(twist)
            self.is_moving = True
            self.get_logger().info(f"Moving towards target. Distance: {self.latest_distance} meters")
        else:
            self.stop_movement()
    ...
    def stop_movement(self):
        ...
        # Stop the timer and reset movement state
        ...
        if self.latest_distance is not None:
            self.get_logger().info(f"Target is within 0.2 meters. Stopping. Distance: {self.latest_distance} meters")
